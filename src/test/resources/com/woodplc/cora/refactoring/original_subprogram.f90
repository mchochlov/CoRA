...
!! whose end node 
!<
subroutine check_pip_contact(int_node,contact)
  ...
  use pipe_in_pipe_data, only : npipnod, nkpipnod, pip_nodes, pip_ignore, pip_contact
  use pipe_in_pipe_data, only: pip_inner_elem, unknown_variable, and_2
  use hddim, only : size_epi
  use maina_and_mainb_arrays, only : epi
  implicit none
  ...
  ! Declare variables that originate in this routine
  integer :: ipair
  integer :: inner_node
  integer :: outer_node
  integer :: nelem
