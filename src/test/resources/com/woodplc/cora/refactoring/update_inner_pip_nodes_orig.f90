  ...
  !<
  subroutine update_inner_pip_outer_nodes(section_limit_exceeded)
  use pipe_in_pipe_data, only : npipsect, pip_section, pip_inner_node, npipnod, pip_nodes, &
    nested_section_limit
  use hddim, only : nonp
  use maina_and_mainb_arrays, only : npnum
  use node_set_data, only : ndset_nod, ndset_defn, ndset_name, nndset
  ...
  integer :: ipair, inode, jnode, user_node_no, jsect
  integer :: section_no, current_section, next_section
  integer :: inner_node_set_no, outer_node_set_no, inner_node, outer_node
  integer :: node_user_to_internal !function
